...
import { addPlayer, startMatch } from "../src/game/match.js";
import {
  difficultyStepFor,
  selectAndWarnPlatforms,
  selectBatch,
  transitionWarningsToGone,
} from "../src/game/platforms.js";
...
describe("platform lifecycle", () => {
  ...
  it("allows selecting occupied platforms", () => {
    const runtime = makeRuntime();
    addPlayerAt(runtime, "p1", "P1", "3:3");
    // With a single survivor there is no survivor protection, so the occupied
...
describe("seeded spawning", () => {
  it("produces unique deterministic spawns", () => {
    const runtime = makeRuntime({ e2eMode: true });
    runtime.phase = "lobby";
