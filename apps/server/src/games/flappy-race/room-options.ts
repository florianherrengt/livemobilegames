 ...
 * off to a Flappy Race room. The roster is server-issued: it comes from the
 * lobby's synchronized player rows, never from a client payload.
 */
export const flappyRaceRoomOptionsSchema = z.object({
  ...
  players: z
    ...
    .min(1)
    .max(FLAPPY_RACE_SERVER_CONSTANTS.MAX_PLAYERS),
  /** Trusted test-mode flag; shortens countdown, result, and course timings. */
...
export type FlappyRaceRoomOptions = z.infer<typeof flappyRaceRoomOptionsSchema>;
