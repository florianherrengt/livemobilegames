 ...
 * off to a Coin Rush room. The roster is server-issued: it comes from the
 * lobby's synchronized player rows, never from a client payload.
 */
export const coinRushRoomOptionsSchema = z.object({
  ...
  players: z
    ...
    .min(1)
    .max(COIN_RUSH_SERVER_CONSTANTS.MAX_PLAYERS),
  /** Trusted test-mode flag; shortens countdown, result, and respawn timings. */
...
export type CoinRushRoomOptions = z.infer<typeof coinRushRoomOptionsSchema>;
