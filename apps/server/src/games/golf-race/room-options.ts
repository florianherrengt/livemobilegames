 ...
 * off to a Golf room. The roster is server-issued: it comes from the lobby's
 * synchronized player rows, never from a client payload.
 */
export const golfRaceRoomOptionsSchema = z.object({
  ...
  players: z
    ...
    .min(1)
    .max(GOLF_SERVER_CONSTANTS.MAX_PLAYERS),
  /** Trusted test-mode flag; shortens countdown, aim, and immunity timings. */
...
export type GolfRaceRoomOptions = z.infer<typeof golfRaceRoomOptionsSchema>;
