 ...
 * off to a Kart Racing room. The roster is server-issued: it comes from the
 * lobby's synchronized player rows, never from a client payload.
 */
export const kartRacingRoomOptionsSchema = z.object({
  ...
  players: z
    ...
    .min(1)
    .max(KART_RACING_SERVER_CONSTANTS.MAX_PLAYERS),
  /** Trusted test-mode flag; shortens countdown, results, and race timings. */
...
export type KartRacingRoomOptions = z.infer<typeof kartRacingRoomOptionsSchema>;
