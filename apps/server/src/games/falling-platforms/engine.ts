...
import { createMatchRng, generateMatchSeed } from "./rng.js";
...
export function startMatch(runtime: MatchRuntime, now: number): boolean {
  ...
  runtime.seed = generateMatchSeed(runtime.settings.e2eMode);
  ...
  participants.forEach((player, index) => {
    ...
    player.alive = true;
    player.jumping = false;
    player.currentPlatformId = spawns[index] ?? "0:0";
    player.fromPlatformId = "";
    player.targetPlatformId = "";
    player.jumpStartedAt = 0;
    player.jumpEndsAt = 0;
...
/**
 * Clears the completed match and resets the round counter so a Play again
 * starts round 1 of a fresh match.
 */
export function resetForNewMatch(runtime: MatchRuntime): void {
  returnToLobby(runtime);
 ...
 * still finish. The room owns the player row and host transfer.
 */
