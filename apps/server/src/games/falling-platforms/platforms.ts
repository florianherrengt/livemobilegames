import type { DifficultyStep } from "@phone-party/protocol";
 ...
 * warning. Never picks the same platform twice in one batch. When more than
 * one grounded survivor exists, the batch leaves at least one of their current
...
export function selectBatch(
  ...
  if (
    ...
    stableIds.includes(FALLING_PLATFORMS_SERVER_CONSTANTS.E2E_FIRST_TARGET) &&
    !selected.includes(FALLING_PLATFORMS_SERVER_CONSTANTS.E2E_FIRST_TARGET) &&
    selected.length < shuffled.length
  ) {
    selected[selected.length - 1] = FALLING_PLATFORMS_SERVER_CONSTANTS.E2E_FIRST_TARGET;
  }
