...
if [[ "${public_url}" != http://* && "${public_url}" != https://* ]]; then
  echo "Coolify returned an invalid public URL: ${public_url}" >&2
  exit 1
fi

curl --silent --show-error --fail --connect-timeout 10 --max-time 20 "${COOLIFY_TUNNEL_ORIGIN}/api/health" | jq -e '.status == "ok"' >/dev/null
...
echo "Cloudflare tunnel origin: ${COOLIFY_TUNNEL_ORIGIN}/api/health (ok)"
