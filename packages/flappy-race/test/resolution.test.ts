...
import { resolveRound } from "../src/resolution.js";
import type { RoundProgressCandidate } from "../src/types.js";
...
describe("round resolution", () => {
  it("award the furthest player when one player crashes earlier", () => {
  ...
  it("draws three or more players crashing on the same obstacle", () => {
    ...
    expect(resolution.winnerSessionIds.sort()).toEqual(["a", "b", "c"]);
  ...
  it("excludes players eliminated on earlier obstacles from a later draw", () => {
    ...
    expect(resolution.winnerSessionIds).toEqual(["d"]);
  });
  ...
  it("draws when a sole survivor crashes before exceeding the highest progress", () => {
    const resolution = resolveRound([candidate("a", 11, false), candidate("b", 11, false)]);
    expect(resolution.winnerSessionIds.sort()).toEqual(["a", "b"]);
  });

  it("wins immediately when a sole survivor passes one additional obstacle", () => {
    const resolution = resolveRound([candidate("a", 11, false), candidate("b", 12, true)]);
    expect(resolution).toMatchObject({ outcome: "resolved", winnerSessionIds: ["b"] });
  });
  ...
  it("keeps winner lists unique", () => {
    ...
    expect(new Set(resolution.winnerSessionIds).size).toBe(resolution.winnerSessionIds.length);
    expect(resolution.winnerSessionIds).toContain("a");
    expect(resolution.winnerSessionIds).not.toContain("d");
  });
  ...
  it("never includes a disconnected player as a winner", () => {
    ...
    expect(resolution.winnerSessionIds).toEqual(["b"]);
  });
