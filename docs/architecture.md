...
installed games, Capital Pin, Falling Platforms, Flappy Race, and Memory Path. The catalogue
is no longer empty, and the lobby transitions into a registered game room when
...
tests; Memory Path owns its manifest, movement command, synchronized route
state, curated path templates, authoritative movement/fall/flash engine,
canvas renderer, and tests. `play_again` is the host-only convention the games use to return to a
